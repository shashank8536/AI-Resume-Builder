...
export default function Builder() {
  ...
  const handleAIGenerate = async () => {
    if (!formData.role || !formData.skills || !formData.experience || !formData.projects) {
      alert("Please fill in your Role, Skills, Experience, and Projects first so the AI has context!");
      return;
    }
    setIsGeneratingAI(true);
    try {
      const response = await fetch('http://localhost:5000/api/ai/enhance-resume', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        alert("Failed to generate AI enhancements.");
      }
    } catch (error) {
      console.error(error);
