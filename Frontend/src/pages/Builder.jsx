...
export default function Builder() {
  ...
  const handleEnhanceSingleField = async (fieldName) => {
    if (!formData[fieldName]) {
      alert(`Please write something in ${fieldName} first before enhancing!`);
      return;
    }
    setEnhancingFields(prev => ({ ...prev, [fieldName]: true }));
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/ai/enhance-field`, {
        method: 'POST',
        headers: {
      ...
      if (data.success && data.enhancedText) {
        setFormData(prev => ({
          ...prev,
          [fieldName]: data.enhancedText
        }));
      } else {
        alert(`Failed to enhance ${fieldName}.`);
      }
    } catch (error) {
      console.error(error);
      ...
      <div className="flex flex-col">
        ...
        <div className="border border-gray-100 p-6 md:p-8 rounded-2xl shadow-lg bg-white">
          <FormSection 
            ...
            onEnhanceField={handleEnhanceSingleField}
            enhancingFields={enhancingFields}
          />
        </div>
      </div>
